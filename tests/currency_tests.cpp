...
int main(int argc,char**argv){
 ...
 if(rates){auto value=convert_currency({5,"btc","xmr"},*rates);check(value.status==CalcStatus::value&&std::abs(value.value-1250)<1e-8&&value.text==L"1250 XMR","cross crypto conversion");check(convert_currency({100,"usd","jpy"},*rates).value==15000,"fiat conversion");check(convert_currency({1,"zzz","usd"},*rates).status==CalcStatus::error,"unsupported code");check(convert_currency({std::numeric_limits<double>::infinity(),"usd","jpy"},*rates).status==CalcStatus::error,"infinite amount");check(convert_currency({1e308,"usd","jpy"},*rates).status==CalcStatus::error,"overflow output");}
